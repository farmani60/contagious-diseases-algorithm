...
ORDER = ["CDA", "CDA (paper)", "CDA (budgeted)", "PSO", "GA", "DE", "Random"]
...
def paper_tables() -> str:
    rows = read("paper_functions.csv")
    if not rows:
        return "_Run `experiments/run_paper_functions.py` first._"

    true_optimum = {"paper_f1": "-19.966683", "paper_f2": "-0.247405"}

    blocks = []
    for function in ("paper_f1", "paper_f2"):
